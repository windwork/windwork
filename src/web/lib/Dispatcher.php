 ...
 * @link        http://docs.windwork.org/manual/wf.web.dispatcher.html
 * @since       1.0.0
 */
class Dispatcher {
    /**
    ...
    /**
     * 
     * @param \wf\web\Application $app
     * @return \wf\web\Dispatcher
     */
    public function setApp(\wf\web\Application $app) {
        $this->app = $app;
    ...
    /**
     * @param \wf\web\Controller $controller
     * @return \wf\web\Dispatcher
     */
    public function setController(\wf\web\Controller $controller) {
    ...
    public function dispatch($uriArg = '') {        
        ...
        \wf\core\Benchmark::mark('dspControllerFore');
        
        // 控制器实现类
        $ctlCalss = $this->router->ctlClass;
        if (!class_exists($ctlCalss)) {
            throw new \wf\web\NotFoundException;
        }
        
        $this->controller = new $ctlCalss($this);
        
        // 控制器继承约束
        if(!$this->controller instanceof \wf\web\Controller) {
            throw new \wf\web\NotFoundException;
        }
        ...
        \wf\core\Benchmark::mark('dspActionFore');
        ...
        \wf\core\Benchmark::mark('dspResponseAft');
     ...
     * @param \wf\web\Application $app
     */
    public function __construct(\wf\web\Application $app) {
        $this->setApp($app);
        
        // http请求初始化
        $this->request  = new \wf\web\Request();
        
        // http响应实例
        $this->response = new \wf\web\Response();
        ...
        if (PHP_SAPI == 'cli') {
            ...
            if (!preg_match("/^(http[s]?:\\/\\/.+?)(\\/.*)/i", $urlCfg['siteUrl'], $siteUrlMatch)) {
                throw new \wf\web\Exception('请在config/url.php中设置siteUrl选项');
            }
