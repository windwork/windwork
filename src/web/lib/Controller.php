 ...
 * @link        http://docs.windwork.org/manual/wf.web.controller.html
 * @since       0.1.0
 */
abstract class Controller
{
     ...
     */
    public function __construct(\wf\web\Dispatcher $dispatcher)
    {
     ...
     * 
     * @param array $params
     * @throws \wf\web\Exception
     */
    public function invokeAction(array $params)
    {
        if (!$this->inited) {
            throw new \wf\web\Exception('请在'.get_called_class().'::__construct()调用parent::__construct()');
        }
    ...
    protected function showMessage($htmlTpl = '')
    {
        ...
        if ($this->request->isAjaxRequest()) {
            print \wf\web\Output::jsonCallable($this->message()->toArray());
            return;
        }
