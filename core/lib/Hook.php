 ...
 *   
 * 2、钩子类必须实现 \wf\core\HookInterface接口
 * 
 * 3、钩子在框架中调用的位置有：     
 ...
 * @link        http://docs.windwork.org/manual/wf.core.hook.html
 * @since       0.1.0
 */
final class Hook 
{
     ...
     * @param mixed $args = null 传递参数
     * @throws \wf\core\Exception
     * @return void|boolean
     */
    protected function callDetail($hook, $args = null)
    {
        $params = $args;
        
        if (is_object($hook)) {
            // 设置值为实例
            if (!$hook instanceof \wf\core\HookInterface) {
                throw new \DomainException(get_class($hook) . '钩子必须实现\wf\core\HookInterface接口');
            }
            ...
            if (isset($this->instances[$hook])) {
                ...
                if (!$hookObj instanceof \wf\core\HookInterface) {
                    throw new \DomainException($hook . '钩子必须实现\wf\core\HookInterface接口');
                }
            ...
            if (is_object($hook[0])) {
                ...
                // 设置值为实例
                if (!$hookObj instanceof \wf\core\HookInterface) {
                    throw new \DomainException(get_class($hookObj) . '钩子必须实现\wf\core\HookInterface接口');
                }
                ...
                if (isset($this->instances[$hookClass])) {
                    ...
                    if (!$hookObj instanceof \wf\core\HookInterface) {
                        throw new \DomainException($hookClass . '钩子必须实现\wf\core\HookInterface接口');
                    }
